...
public class DataMapperMediator extends AbstractMediator {
    ...
    /**
     * Gets the local name of the mediator
     * @return local name of the mediator
     */
	...
	/**
	 * Serializer for the DataMapperMediator which converts the DataMapperMediator
	 * properties mentioned in UI to xml configuration
	 * @return OMElement the datamapper mediator element
	 */
	...
	public OMElement serialize(OMElement parent) {
		...
		if (configurationKey != null) {
			...
			keySerializer.serializeValue(configurationKey,
					ATT_CONFIGURATION_KEY, dataMapperElement);
		} else {
		...
		if (inputSchemaKey != null) {
			ValueSerializer keySerializer = new ValueSerializer();
			keySerializer.serializeValue(inputSchemaKey, ATT_INPUT_SCHEMA_KEY,
					dataMapperElement);
		} else {
		...
		if (outputSchemaKey != null) {
			ValueSerializer keySerializer = new ValueSerializer();
			keySerializer.serializeValue(outputSchemaKey,
					ATT_OUTPUT_SCHEMA_KEY, dataMapperElement);
		} else {
		...
		if(inputType != CSV_VALUE ){
			...
			dataMapperElement.addAttribute(fac.createOMAttribute(ATT_INPUT_TYPE, nullNS, CSV));
		}
		...
		if(outputType != CSV_VALUE ){
			...
			dataMapperElement.addAttribute(fac.createOMAttribute(ATT_OUTPUT_TYPE, nullNS, CSV));
		}
	...
	/**
	 * Retrieves the value from xml configuration and display in the design view
	 */
