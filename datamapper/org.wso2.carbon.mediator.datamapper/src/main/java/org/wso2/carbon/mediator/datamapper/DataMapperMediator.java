 ...
 */
public class DataMapperMediator extends AbstractMediator implements
		ManagedLifecycle {
	...
	private final static  Log log = LogFactory.getLog(DataMapperMediator.class);
	 ...
	 * 
	 * @return the key which is used to pick the input schema from the registry
	 */
	 ...
	 * 
	 * @return the key which is used to pick the output schema from the registry
	 */
	 ...
	 *            current message for the mediation
		...
		if (!(StringUtils.isNotEmpty(configkey)
			...
			try {
				...
				synchronized (DataMapperHelper.class) {
					DataMapperHelper.transform(messageContext, configkey,
							inSchemaKey, outSchemaKey, inputType, outputType,
							getUniqueID());
				}
			} catch (SynapseException syExp) {
				log.error("Mediation failed at DataMapperMediator", syExp);
				result = false;
			} catch (IOException e) {
				log.error("Mediation failed at DataMapperMediator", e);
				result = false;
			}
		}
	 ...
	 * 
	 * @param se
	 *            SynapseEnvironment used at initialization
	 */
	@Override
