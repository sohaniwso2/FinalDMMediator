...
public class XMLWriter implements OutputWriter {
	...
	/*
	 * public static OMElement getInputMessage(String inputType,OMElement
	 * inputMessage) throws IOException {
	 * 
	 * DatumReader<GenericRecord> reader = null; Decoder decoder = null;
	 * OMElement inMessage = null; try { ByteArrayInputStream
	 * byteArrayInputStream = new ByteArrayInputStream(inputType.getBytes());
	 * reader = ReaderRegistry.getInstance().get(inputType).newInstance();
	 * reader.setSchema(mappingResourceLoader.getInputSchema()); decoder = new
	 * DummyDecoder(byteArrayInputStream); reader.read(reuse, decoder);
	 * 
	 * 
	 * } catch (Exception e) { handleException("Data coversion Failed", e); }
	 * finally { decoder.flush(); } return inputMessage;
	 * 
	 * 
	 * }
	 */
